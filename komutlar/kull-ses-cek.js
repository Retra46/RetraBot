...
exports.run = async(client, message, args) => {
    ...
    if (!message.member.voice.channel) {
        return message.reply("Herhangi Bir Sesli Kanalda Olman lazım .");
    }
    ...
    if (!kullanıcı) return message.channel.send("` **LÜtfen Bir Kullanıcı Etiketleyin .**`");

    let member = message.guild.member(kullanıcı);

    if (!member.voice.channel) return message.channel.send('Etiketlediğiniz Kullanıcı Ses Kanalında Değil.').then(m => m.delete, { timeout: 5000 });
