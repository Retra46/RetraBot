...
exports.run = async (client, message, args) => {
  ...
  if (!args[0] || isNaN(args[0])) {
    const temizle = new Discord.MessageEmbed()
      ...
      .setDescription(
        `Temizlenecek Mesaj Miktarını Belirtmelisin!\n\n Örnek Kullanım : ${prefix}**temizle 10**`
      )
      .setColor("RED");
  ...
  const sil = new Discord.MessageEmbed()
    ...
    .setDescription(
      `**\`\`${
        args[0]
      }\`\` Adet Mesaj Uzay Boşluğuna Yuvarlandı <:onayy:809839478561570888> Off Çok yoruldum <a:onayytikk:811491281883627532>   **`
    )
    .setColor("GREEN");
