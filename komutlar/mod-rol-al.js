...
if (!role) return message.channel.send('Rol bulunamadı.');
  ...
  return message.channel.send(`<:iptal:811555198467047444> Alınacak rol sizin rolünüzün üstünde bu yüzden rolü **Veremiyorum!**`);
  }
  try{
await (member.roles.remove(role.id))
 message.channel.send(new emran.MessageEmbed().setDescription(`${member} isimli üyenin \`${role.name}\` isimli yetkisi başarıyla alındı <:onayy:809839478561570888>`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('#D2EE07'));
